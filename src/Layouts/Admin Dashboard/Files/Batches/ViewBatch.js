...
import { getRegistrationsSpecificStudent } from '../Apis/apiForRegistrations';
...
function ViewBatch() {
    ...
    useEffect(() => {
      const fetchItems = async function() {
        const teachers = await getRegistrationsSpecificStudent(location.state.batchTitle)
        setItems(teachers)
      }
      fetchItems()
    }, []);
    ...
    <div className="containerBlackDashboard-fluid mt-5">
      ...
      <div className="card align-middle justify-content-center m-auto shadow-sm  col-xl-10 col-lg-9 col-md-8  border-0 mb-4">
        <div className="my-3" style = {{color : "rgba(55, 64, 85, 0.9)"}}>
          <h5 className="mb-2 lead display-5 text-center" style={{ color:'rgba(55, 64, 85, 0.9)', fontWeight:'900' }}>List Of {/*{location.state.gender}*/} Students in Batch: {location.state.batchTitle}</h5>
        </div>
