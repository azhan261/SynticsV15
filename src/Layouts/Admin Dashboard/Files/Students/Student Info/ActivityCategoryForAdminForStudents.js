...
function ActivityCategoryForAdminForStudents() {
    let {id} = useParams()
    ...
    const onSubjectClick = (e) => {
      e.preventDefault(e)
      history.push({
          pathname:`/admin/student-activity/${id}`,
    ...
    const onTestClick = (e) => {
      e.preventDefault(e)
      history.push({
        pathname:`/admin/student-activity-test/${id}`,
        state: location.state
      })
    }
    const onAssignmentClick = (e) => {
      e.preventDefault(e)
      history.push({
        pathname:`/admin/student-activity-assignments/${id}`,
        state: location.state
      })
    }
      return (
  ...
  {/* Page Heading */}
  <h1 className='text-center display-4 my-3' style={{ color:'rgba(55, 64, 85, 0.9)', fontWeight:'900' }}>Category of Student Activity</h1>
...
export default ActivityCategoryForAdminForStudents
