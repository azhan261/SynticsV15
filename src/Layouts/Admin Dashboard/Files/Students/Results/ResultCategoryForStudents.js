...
function ResultsCategoryForStudentsAdmin() {
  let {id} = useParams()
  ...
  const onTestClick = (e) => {
    e.preventDefault(e)
    history.push({
      pathname: `/admin/answer-list-test/${id}`,
      state: location.state
    })
  }
  ...
  const onAssignmentClick = (e) => {
    e.preventDefault(e)
    history.push({
      pathname: `/admin/answer-list-assignments/${id}`,
      state: location.state
    })
  }
    return (
...
export default ResultsCategoryForStudentsAdmin
