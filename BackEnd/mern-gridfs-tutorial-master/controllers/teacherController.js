...
router.post("/specific/id/:id", (req, res) => {
  ...
  router.post('/admin/batch/:batch', (req, res) => {
    console.log("test")
    const batch = req.params.batch;
    Teacher.find({ "subjectChoosenForApplication" : batch} , (err, student) => {
      res.json(student);
    });
  }); 
  
  router.post('/admin/batch/projectManager/:name', (req, res) => {
    console.log("test")
    const name = req.params.name;
    Teacher.find({ "name" : { '$ne': name }} , (err, student) => {
      res.json(student);
    });
  }); 
 ...
 router.post("/:id", (req, res) => {
    ...
    Teacher.findById(id, (err, teacher) => {
      if (!teacher) {
        ...
        teacher.batchTitle = req.body.batchTitle,
        teacher.batchID = req.body.batchID,
        teacher
          .save()
          .then((teacher) => {
            res.json(teacher);
